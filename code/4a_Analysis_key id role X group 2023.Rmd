...
```{r}
# fall groups
rankings <- read.csv("../results/bordaranking_scaled_fall23_93degus.csv") %>% 
  select(-X, -X.1) %>% 
  arrange(BordaScore) %>% 
  mutate(rank= 1:n()) 
...
groups <- read.csv("../results/idcharacteristics23.csv") %>% 
  dplyr::select(id, sex, groupid)
glimpse(groups)
...
degulist23<- rankings %>% 
  left_join(groups) %>% 
  filter(!groupid %in% c(NA, "na")) %>% 
  rename(group = groupid)

n_distinct(degulist23$group)
```
Exclude groups of 1 degu
Add key degus

```{r}
groupingsselect <- degulist23 %>% 
  group_by(group) %>% 
  tally() %>% 
  filter(n!=1) 

# group size
groupsize <- degulist23 %>% 
  group_by(group) %>% 
  tally() %>% 
  rename(groupsize=n)
...
degus23<- degulist23 %>% 
  ...
  mutate(topkey= if_else(Borda<11, "key", "nonkey"),
         thkey=if_else(Borda<15, "key", "nonkey"),  # cutoff -> 15 key individuals
         qkey= if_else(BordaScore<quantile(BordaScore, probs = 0.25), "key", "nonkey"))

n_distinct(degus23$group)
...
## dyadlist
of degus with ranking and group membership 
```{r}
id <- degus23$id 
...
#compare network metrics by group

based on threshold

## 1.	calculate group-level network metrics per group instead of the complete network
...
```{r}
 networkmetricsXgroup23 <-  bind_rows(networkmetricsXgroup_prox,
                                      networkmetricsXgroup_aff,
                                      networkmetricsXgroup_agg,
                                      multiplexmetricsXgroup)  %>% 
  mutate(year="2023") 
write.csv(networkmetricsXgroup23,
          "../results/networkmetricsXgroup_fall23_excludesnulls_v1.csv")
...
keysex23$group <- as.factor(keysex23$group)
keysex23$sex <- as.factor(keysex23$sex)
networkmetricsXgroup23$group <- as.factor(networkmetricsXgroup23$group)


metricsXkey.sex <- networkmetricsXgroup23 %>% 
  left_join(keysex23) %>% 
  drop_na(sex) 

metricsXkey.sex23 <- metricsXkey.sex

write.csv(metricsXkey.sex,
          "../results/networkmetricsXgroupXsex_fall23.csv")
```

## 2.	compare the network metrics of groups with and without key individuals 

```{r}

selectnetwork <- "proximity" # "proximity" "affiliative" "agonistic"

networkmetricsXgroup23$group <- as.integer(networkmetricsXgroup23$group) 
networkmetricsXgroupXkey <- networkmetricsXgroup23 %>% 
  left_join(groups.key)
...
wilcox.test(diameter~group_with_thkey, data=ref.data)
...
#avg_path_length.obs <- coefficients(lm(avg_path_length ~ group_with_thkey, data = ref.data))[2]
...
m1<- glm(avg_path_length ~ group_with_thkey, data = ref.data, family=Gamma("inverse"))
m2 <- lm(avg_path_length ~ group_with_thkey, data = ref.data)
...
range(multiXkey$m.apl, na.rm=T)

# normally distributed?
ggplot(multiXkey, aes(x=m.apl)) +
  geom_histogram()  # very right skewed

  # shapiro test p >0.05, data normally distributed
#ggqqplot(trap, "time_s")
shapiro.test(multiXkey$m.apl)


m <- lm(m.apl ~ group_with_thkey, data = multiXkey)
#m<- glm(avg_path_length ~ withkey, data = ref.data, family=Gamma("inverse"))
summary(m)
d <- data.frame(residuals = residuals(m),         # Residuals
                std_residuals = stats::rstudent(m), # Studentized Residuals
...
# distribution
fitdistrplus::plotdist(multiXkey$m.apl)
#fitdistrplus::descdist(ref.data$avg_path_length, discrete = F) # beta distribution

ggplot(multiXkey, aes(x=group_with_thkey, y=m.apl)) +
  geom_boxplot() +
  theme_classic()


m1<- glm(m.apl ~ group_with_thkey, data = multiXkey, family=Gamma("inverse"))
m2 <- lm(m.apl ~ group_with_thkey, data = multiXkey)
...
#avg_path_length.obs <- coefficients(glm(avg_path_length ~ group_with_key, data = multiXkey, family=Gamma("inverse")))[2]
#avg_path_length.obs <- coefficients(glm(m.apl ~ group_with_thkey, data = multiXkey, family=Gamma("inverse")))[2]
#avg_path_length.obs <- coefficients(glm(avg_path_length ~ group_with_qkey, data = multiXkey, family=Gamma("inverse")))[2]

avg_path_length.obs <- coefficients(lm(m.apl ~ group_with_thkey, data =multiXkey))[2]
...
# shapiro test p >0.05, data normally distributed
#ggqqplot(trap, "time_s")
shapiro.test(multiXkey$cluster)
shapiro.test(ref.data$cluster)


m <- lm(cluster ~ group_with_thkey, data = multiXkey)
summary(m)
d <- data.frame(residuals = residuals(m),         # Residuals
                std_residuals = stats::rstudent(m), # Studentized Residuals
                fitted = fitted(m),          # Fitted values
...
multiXkey$cluster[multiXkey$cluster==0.0]<-0.0001

m1<- betareg(cluster ~ group_with_thkey, data = multiXkey)
m2 <- lm(cluster ~ group_with_thkey, data = multiXkey)
...
#coefficients(betareg(density ~ group_with_thkey, data = multiXkey))[2]
...
multilayer.obs<- cbind.data.frame(network, 
                                  ...
                                  cluster.obs,
                                  overlap.obs)
write.csv(multilayer.obs, "../results/observed multilayer metrics groups23_threshold key ids.csv")
```

### combine results
```{r}
observed.summary23 <- bind_rows(prox.obs, aff.obs, agg.obs)
...
job::job({
run.s <- as.character(unique(ref.model.prox$runID))
...
```{r summarize ref model - agonistic}

job::job({
run.s <- as.character(unique(ref.model.agg$runID))
replicates <- 1000 #how many runs of the reference model were done
...
plot_prox_count_p <- ggplot(ref.summary.prox, aes(x=count.ref)) +
  ...
  theme_classic() +
  labs(x= "Count coefficient value ", y="Frequency", title = "P = 0.60") +
...
plot_prox_diameter <- ggplot(ref.summary.prox, aes(x=diameter.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Diameter coefficient value ", y="Frequency", title = "P = 0.84") +
  #geom_vline(xintercept=observed.summary$diameter.obs[observed.summary$network=="proximity"], color="red") +
...
plot_prox_density <-  ggplot(ref.summary.prox, aes(x=density.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Density coefficient value", y="Frequency", title = "P = 0.20") +
...
plot_prox_apl_p <- ggplot(ref.summary.prox, aes(x=avg_path_length.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Average path length coefficient value", y="Frequency", title="P = 0.09") +
...
plot_prox_eff_p <-   ggplot(ref.summary.prox, aes(x=efficiency.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Efficiency coefficient value", y="Frequency", title="P = 0.63") +
...
plot_aff_count_p <-  ggplot(ref.summary.aff, aes(x=count.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Count coefficient value", y="Frequency", title="P = 0.33") +
...
plot_aff_diameter <-  ggplot(ref.summary.aff, aes(x=diameter.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Diameter coefficient value", y="Frequency", title="P = 0.59") +
...
plot_aff_density <- ggplot(ref.summary.aff, aes(x=density.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Density coefficient value", y="Frequency", title="P = 0.09") +
...
plot_aff_den <- networkmetricsXgroupXkey %>% 
 ...
 plot_aff_apl_p <- ggplot(ref.summary.aff, aes(x=avg_path_length.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Average path length coefficient value", y="Frequency", title="P = 0.39") +
...
plot_aff_eff_p <- ggplot(ref.summary.aff, aes(x=efficiency.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Efficiency coefficient value", y="Frequency", title="P = 0.88") +
...
plot_agg_count_p <- ggplot(ref.summary.ago, aes(x=count.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Count coefficient value", y="Frequency", title="P = 0.02") +
...
plot_agg_diameter <- ggplot(ref.summary.ago, aes(x=diameter.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Diameter coefficient value", y="Frequency", title="P = 0.20") +
...
plot_agg_density <-  ggplot(ref.summary.ago, aes(x=density.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Density coefficient value", y="Frequency", title="P = 0.07") +
...
plot_agg_apl_p <- ggplot(ref.summary.ago, aes(x=avg_path_length.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Average path length coefficient value", y="Frequency", title="P = 0.47") +
...
sum(observed.summary$efficiency.obs[observed.summary$network=="agonistic"]<ref.summary.ago$efficiency.ref)/length(ref.summary.ago$efficiency.ref)
 plot_agg_eff_p <-  ggplot(ref.summary.ago, aes(x=efficiency.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Efficiency coefficient value", y="Frequency", title="P = 0.0") +
...
plot_multi_density <-  ggplot(ref.summary.multi, aes(x=density.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Density coefficient value", y="Frequency", title="P = 0.25") +
...
# average path length
  ...
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Average path length coefficient value", y="Frequency", title="P = 0.51") +
...
sum(multilayer.obs$cluster.obs<ref.summary.multi$cluster.ref)/length(ref.summary.multi$cluster.ref)
 plot_multi_clus_p <-  ggplot(ref.summary.multi, aes(x=cluster.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Cluster coefficient value", y="Frequency", title="P = 0.72") +
...
plot_multi_clus <- multiXkey %>% 
 ...
 plot_multi_ove_p <-  ggplot(ref.summary.multi, aes(x=overlap.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Overlap coefficient value", y="Frequency", title="P = 0.29") +
...
```{r}
plot_networkmetricsXgroup23 <- ggpubr::ggarrange(plot_prox_count + rremove('xlab'), 
...
plot_networkmetricsXgroup23
ggsave("../figures/thkeyXgroupXmetrics_fall23.pdf",width=180, height = 210, units = "mm", dpi = 300)                  
```
```{r all}
plot_networkmetricsXgroup23_all <- ggpubr::ggarrange(plot_prox_count + rremove('xlab'), 
                  plot_aff_count + rremove('xlab')+ rremove('ylab'), 
                  plot_agg_count + rremove('xlab') + rremove('ylab'), 
...
plot_networkmetricsXgroup23_all
ggsave("../figures/thkeyXgroupXmetrics_fall23_all.pdf",width=180, height = 210, units = "mm", dpi = 300)                  
```

##5. Visualize monolayer networks X group
...
keysex23$group <- as.factor(keysex23$group)
groupsize$group <-as.factor(groupsize$group)
degulist23$group <-as.factor(degulist23$group)

groupchars23 <- degulist23 %>% 
  group_by(group, sex) %>% 
  tally() %>% 
  ...
  left_join(groupsize) %>% 
  left_join(keysex23, by="group") %>% 
  mutate(M_key=if_else(sex.keyID=="M", "yes", "no"),
         F_key=if_else(sex.keyID!="M", "yes", "no"))

groupchars23$M_key[is.na(groupchars23$M_key)] <- "no"
groupchars23$F_key[is.na(groupchars23$F_key)] <- "no"
...
ggplot(summary_df, aes(x=male_group, y=n, fill=is_key)) +
...
my_table <- matrix(c(8, 6, 4, 9), nrow = 2, byrow = TRUE)
# Row names and column names can represent the categories
...
m.males <- glmer( male_key ~ M + (1|group),
                  data = df,
                  family = binomial
                  )
summary(m.males)

m.malescat <- glmer( male_key ~ male_group + (1|group),
                  data = df,
                  family = binomial
                  )
summary(m.malescat)
...
m.males1 <- glmer( male_key ~ 1 + (1|group),
                  data = df,
                  family = binomial
                  )
summary(m.males1)
...
```
We did not find a relationship between whether a male is key and number of males in the group (Fisher exact test: odds ratio = 2.88, p = 0.25)
...
see Rmd 5_Visualise results

# save work

```{r}
save.image("../results/key_roleXgroup_fall2023.Rdata")
```
