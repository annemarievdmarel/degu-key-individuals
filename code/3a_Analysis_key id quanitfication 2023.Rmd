...
#degus_characteristics23$groupid[degus_characteristics23$groupid=="13a"] <- 130  # done in excel
n_distinct(degus_characteristics23$groupid)
table( degus_characteristics23$age_yrs)

rankings_fall <- read.csv("../results/bordaranking_scaled_fall23_93degus.csv") %>%  
  dplyr::select(-X.1) %>% 
...
degu_keys <- rankings_fall %>% 
   left_join(degus_characteristics23, by=c("id")) %>% 
    mutate(colorx = ifelse(sex=="F",'#5D3A9B', "#E66100"),
         qkey=if_else(BordaScore<lowerq_st$q, "yes", "no"),
         thkey=if_else(Borda<15, "yes", "no"),  # cutoff -> 14 key individuals
...
rankXchars_nona <- rankXchars_nona %>% 
  ...
  na.omit()
# 75 degus with all chars of the 93
...
df <- rankXchars_nona %>% 
  dplyr::select(meanagd, mean_latency, bodycondition, hr50) %>% 
  na.omit()

df <- degu_rankings %>% 
  dplyr::select(meanagd, mean_latency, hr50) # remove body condition
...
keychars <- rankXchars_nona %>% 
  ...
  #dplyr::select(thkey, sex, meanagd, bodycondition, mean_latency, hr50, zone, groupid)

m.topkey <- glmer( key_num ~ sex*meanagd+ bodycondition+ mean_latency+ hr50+ (1|zone),
                  data = keychars,
                  family = binomial
                  )
summary(m.topkey)
...
m.topkey1 <- glmer( key_num ~ sex+ masculinization + bodycondition+ docility+ hr50+ (1|zone),
...
m.thkey <- glmer( key_num ~ sex*meanagd+ bodycondition+ mean_latency+ hr50+ (1|zone),
                  data = na.omit(keychars),
                  family = binomial
                  )
summary(m.thkey)
...
m.thkey1 <- glmer( key_num ~ sex+ masculinization + bodycondition+ docility+ hr50+ (1|zone),
                  data = na.omit(keychars),
                  family = binomial
                  )
summary(m.thkey1)
...
m.qkey <- glmer( key_num ~ sex*meanagd+ bodycondition+ mean_latency+ hr50+ (1|zone),
                  data = keychars,
                  family = binomial
                  )
summary(m.qkey)
...
m.qkey1 <- glmer( key_num ~ sex+ masculinization + bodycondition+ docility+ hr50+ (1|zone),
                  data = na.omit(keychars),
                  family = binomial
                  )
summary(m.qkey1)
...
chisq.test(degu_rankings$qkey, degu_rankings$sex)

ggplot(rankXchars_nona, aes(x=sex, fill=qkey)) +
...
ggplot(rankXchars_nona, aes(y=BordaScore, x=sex, color=sex)) +
...
```{r}
# mean agd
ggpubr::ggqqplot(degu_rankings, "meanagd")# shapiro test p >0.05, data normally distributed
shapiro.test(degu_rankings$meanagd)
chisq.test(degu_rankings$qkey, degu_rankings$meanagd)

ggplot(rankXchars_nona, aes(y=meanagd, x=thkey, color=sex)) +
  geom_boxplot(position = "dodge") +
  geom_jitter(  width=0.05) +
  facet_grid(~sex) +
  theme_classic() +
  scale_color_manual( values=c("#fdae61", "#0571b0")) +
  labs(y="Anogenital distance (mm)", x= "Key ID") +
  theme(axis.text = element_text(size=18),
        title = element_text(size=20),
        legend.position = "bottom") 

ggplot(rankXchars_nona, aes(x=qkey, y=meanagd, color=sex, group=sex)) +
  geom_violin(width=0.5, color="black")+
  geom_boxplot(width=0.2, color="grey", alpha=0.6) +
  geom_jitter(  width=0.05) +
...
chisq.test(degu_rankings$key, degu_rankings$masculinization)
ggplot(na.omit(degu_rankings), aes(y=BordaScore, x=masculinization, color=masculinization)) +
...
#age
chisq.test(degu_rankings$key, degu_rankings$age_yrs)
ggplot(rankXchars_nona, aes(y=BordaScore, x=as.factor(age_yrs), color=sex)) +
    geom_violin(width=0.5, color="black")+
  geom_boxplot(width=0.2, color="grey", alpha=0.6) +
  geom_jitter(  width=0.05) +
  theme_classic() +
...
# bodycondition 
chisq.test(degu_rankings$key, degu_rankings$bodycondition)

ggplot(rankXchars_nona, aes(y=bodycondition, x=thkey, color=sex)) +
    geom_violin(width=0.5, color="black")+
  geom_boxplot(width=0.2, color="grey", alpha=0.6) +
  geom_jitter(  width=0.05) +
  theme_classic() +
 ...
 ggplot(na.omit(degu_rankings), aes(y=BordaScore, x=bodycondition)) +
  geom_point() +
  theme_classic() +
  scale_y_reverse() +
  geom_smooth(method = "lm", fill = NA) +
...
```{r}
 ...
 # docility
 chisq.test(degu_rankings$key, degu_rankings$docility)
 
ggplot(rankXchars_nona, aes(y=mean_latency, x=thkey, color=sex)) +
    geom_violin(width=0.5, color="black")+
  geom_boxplot(width=0.2, color="grey", alpha=0.6) +
  geom_jitter(  width=0.05) +
  theme_classic() +
...
ggplot(na.omit(degu_rankings), aes(y=BordaScore, x=docility)) +
  geom_boxplot() +
  theme_classic() +
  scale_y_reverse() +
  geom_smooth(method = "lm", fill = NA) +
 ...
 # boldness
 chisq.test(degu_rankings$key, degu_rankings$mean_latency)
 ggplot(na.omit(degu_rankings), aes(y=BordaScore, x=mean_latency)) +
  geom_point() +
  theme_classic() +
  scale_y_reverse() +
  geom_smooth(method = "lm", fill = NA) +
...
# exploration
 chisq.test(degu_rankings$key, degu_rankings$hr50) 

ggplot(rankXchars_nona, aes(y=hr50, x=thkey, color=sex)) +
    geom_violin(width=0.5, color="black")+
  geom_boxplot(width=0.2, color="grey", alpha=0.6) +
  geom_jitter(  width=0.05) +
  theme_classic() +
...
ggplot(na.omit(degu_rankings), aes(y=BordaScore, x=hr50)) +
  geom_point() +
  theme_classic() +
  scale_y_reverse() +
  geom_smooth(method = "lm", fill = NA) +
...
### Reversed Borda Score (continuous) 
right skewed data


```{r gamma distribution-continuous}

m.gam <-  lme4::glmer(bordareverse ~ sex * meanagd + bodycondition + mean_latency + hr50 + (1|zone), 
                      family = Gamma(link = "log"), 
                      data = rankXchars_nona) # singular fit
m.gam1 <-  lme4::glmer(bordareverse ~ sex + meanagd + bodycondition + mean_latency + hr50 + (1|zone), 
                      family = Gamma(link = "log"), 
                      data = rankXchars_nona)
m.gam2 <-  glm(bordareverse ~sex * meanagd + bodycondition + mean_latency + hr50 , 
                      family = Gamma(link = "log"), 
                      data = rankXchars_nona)

m.agd <- gamlss::gamlss(bordareverse~ sex * meanagd + bodycondition + mean_latency + hr50 + (1|zone), 
                    data = rankXchars_nona,
                    family = "GA")  
summary(m.agd)
plot(m.agd)
...
```{r skew normal 2 distribution}
m.sn2 <- gamlss::gamlss(bordareverse ~ sex * meanagd + bodycondition + mean_latency + hr50 + (1|zone), 
                    data = rankXchars_nona,
                    family = "SN2")  
summary(m.sn2)
plot(m.sn2)
...
latency.obs <- coefficients(m)[5]
hr50.obs <- coefficients(m)[6]
sexXagd.obs <- coefficients(m)[8]

observed.coefs <- cbind.data.frame(sex.obs, agd.obs, bc.obs, latency.obs, hr50.obs, sexXagd.obs)
...
fit_observed <- gamlss::gamlss(bordareverse ~ sex * meanagd + bodycondition + mean_latency + hr50 + (1|zone), 
                    data = rankXchars_nona,
                    family = "SN2")
...
for (i in 1:nperm) {
  ...
  fit_perm <- try(gamlss(bordareverse~ sex * meanagd + bodycondition + mean_latency + hr50 + (1|zone),
...
```
Null hypothesis is accepted. Individual attributes do not explain borda score in degus. 
...
for (run in 1:replicates) {
  ...
  print(r.seed)
	...
	# randomize docility
	s <- sample(1:N)
	ref.data$doc_ref <- ref.data$mean_latency[s]
	...
	# re-run model
 m.ref  <- gamlss::gamlss(bordareverse ~ sex_ref * agd_ref + bc_ref + hr_ref + doc_ref + (1|zone), 
                    data = ref.data,
                    family = "SN2")  
...
doc.ref <- coefficients(m.ref)[6]
sexXagd.ref <- coefficients(m.ref)[8]


 runID <- paste0("run",str_pad(r.seed, 3, side="left", pad = "0"))

ref.coefs <- data.frame(runID, sex.ref, agd.ref, bc.ref,doc.ref, hr.ref, sexXagd.ref)
...
p.latency <- sum(observed.coefs$latency.obs<coef.ref$doc.ref, na.rm=T)/length(coef.ref$doc.ref)
p.latency
...
plot.latency <- ggplot(coef.ref, aes(x=doc.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Coefficient value latency", y="Frequency") +
  geom_vline(xintercept=observed.coefs$latency.obs, color="red") +
  geom_vline(xintercept = rep(quantile(na.omit(coef.ref$doc.ref), 0.025), 2), color="darkblue", lty=2) +
  geom_vline(xintercept = rep(quantile(na.omit(coef.ref$doc.ref), 0.975), 2), color="darkblue", lty=2)
plot.latency
...
plot.sexXagd <- ggplot(coef.ref, aes(x=sexXagd.ref)) +
  geom_histogram(color="grey", fill="grey") +
  theme_classic() +
  labs(x= "Coefficient value sex:AGD", y="Frequency") +
...
rank.sex23 <- ggplot(rankXchars_nona, aes(y=BordaScore, x=sex, color=sex)) +
    geom_violin(width=0.5, color="black")+
  geom_boxplot(width=0.2, color="grey", alpha=0.6) +
  geom_jitter(  width=0.05) +
  theme_classic() +
  ...
  theme(#axis.text = element_text(size=18),
        #title = element_text(size=20),
        legend.position = "none") +
  geom_text(x=1.5, y=-7, label=p_sex, color="black", size=3)
rank.sex23 
...
plot_sex23 <- ggpubr::ggarrange(rank.sex23, plot.sex)
...
rank.agd23 <- ggplot(rankXchars_nona, aes(y=BordaScore, x=meanagd, color=sex, group=sex)) +
    #geom_violin(width=0.5, color="black")+
  #geom_boxplot(width=0.2, color="grey", alpha=0.6) +
  geom_jitter(  width=0.05) +
  geom_smooth(method = "lm", fill = NA) +
  theme_classic() +
  scale_y_reverse() +
  scale_color_manual( values = c("F"='#5D3A9B', "M"="#E66100")) + # values=alpha(c("#fdae61", "#0571b0"),1)) +
  labs(y="Borda Score", x="Anogenital distance (mm)") +
  theme(#axis.text = element_text(size=18),
        #title = element_text(size=20),
        legend.position = "none") +
  geom_text(x=4, y=-6.5, label=p_agd, color="black", size=3) +
   geom_text(x=4, y=-7.5, label=p_sexXagd, color="black", size=3)

rank.agd23

plot_p <- ggpubr::ggarrange(plot.agd, plot.sexXagd, ncol=1)

plot_agd23 <- ggpubr::ggarrange(rank.agd23, plot_p, nrow=1)

ggsave("results/plot_rankXagd_v1.jpeg", width=5, height = 4)
```
...
rank.bc23 <- ggplot(rankXchars_nona, aes(y=BordaScore, x=bodycondition)) +
    #geom_violin(width=0.5, color="black")+
  #geom_boxplot(width=0.2, color="grey", alpha=0.6) +
  geom_jitter(  width=0.05) +
  geom_smooth(method = "lm", fill = NA) +
 ...
 #scale_color_manual( values=alpha(c("#fdae61", "#0571b0"),1)) +
  ...
  theme(#axis.text = element_text(size=18),
        #title = element_text(size=20),
        legend.position = "none") +
  geom_text(y=-6.5, x=-0.2, label=p_bc, color="black", size=3)
  
rank.bc23

plot_bc23 <- ggpubr::ggarrange(rank.bc23, plot.bc)
...
rank.lat23 <- ggplot(rankXchars_nona, aes(y=BordaScore, x=mean_latency)) +
    #geom_violin(width=0.5, color="black")+
  #geom_boxplot(width=0.2, color="grey", alpha=0.6) +
  geom_jitter(  width=0.05) +
  geom_smooth(method = "lm", fill = NA) +
  ...
  scale_y_reverse() +
 #scale_color_manual( values=alpha(c("#fdae61", "#0571b0"),1)) +
  labs(y="Borda Score", x="Latency to leave trap (s)") +
  theme(#axis.text = element_text(size=18),
        #title = element_text(size=20),
        legend.position = "none") +
  geom_text(x=1, y=-6.5, label=p_lat, color="black", size=3)

rank.lat23

plot_lat23 <- ggpubr::ggarrange(rank.lat23, plot.latency)
```
...
rank.hr23 <- ggplot(rankXchars_nona, aes(y=BordaScore, x=hr50)) +
  geom_point() +
  theme_classic() +
  scale_y_reverse() +
  geom_smooth(method = "lm", fill = NA) +
  ...
  theme(#axis.text = element_text(size=18),
        #title = element_text(size=20),
        legend.position = "none") +
  geom_text(x=0.02, y=-6.5, label=p_hr, color="black", size=3)

rank.hr23.r <- ggplot(rankXchars_nona, aes(y=bordareverse, x=hr50)) +
  geom_point() +
  theme_classic() +
  #scale_y_reverse() +
  geom_smooth(method = "lm", fill = NA) +
...
ggpubr::ggarrange(rank.hr23, rank.hr23.r)

plot_hr23 <- ggpubr::ggarrange(rank.hr23, plot.hr)
...
plot.rank.23 <- ggpubr::ggarrange(rank.sex23, rank.agd23, rank.bc23, rank.lat23, rank.hr23, 
                  nrow=3, ncol=2,
                   labels="auto"
                   )
plot.rank.23

ggsave("../figures/plot_rankXchars23_v1.pdf",  width=180, height = 180, units = "mm", dpi = 300)
```


```{r}

plot.rank.all23 <- ggpubr::ggarrange(plot_sex23, plot_agd23, plot_bc23, plot_lat23, plot_hr23, 
                  nrow=5, ncol=1,
                   labels="auto"
                   )
plot.rank.all23

ggsave("../figures/plot_rankXchars23_hist.pdf",  width=180, height = 220, units = "mm", dpi = 300)
```
...
```{r}
save.image("../results/key_quantification_fall2023_v2.Rdata")


load(("../results/key_quantification_fall2023_v2.Rdata"))
```
